...
def read_dataframes(files):
    ...
    for file in files:
        ...
        df.columns = pd.MultiIndex.from_product(
            [list(df.columns), [year]], names=['type', 'year'])
        dups = df.index.duplicated()
        if dups.sum() > 0:
            print("... {} from {}".format(dups.sum(), year))
        df = df[~dups]
        try:
...
def main():
    for folder in list(os.walk(SOURCE_FOLDER))[0][1]:
        files = sorted(glob("{}{}/*.csv".format(SOURCE_FOLDER, folder)))

        df = read_dataframes(files)

        # Non-interpolated version
        save_melted(df, "{}{}.csv".format(TARGET_FOLDER, folder))

        # Interpolated version
        df = pd.concat([interpolate(df, 'Rank'), interpolate(df, 'Score')],
                       axis=1)
        save_melted(df, "{}{}_interpolated.csv".format(TARGET_FOLDER, folder))

        print(">>> Saving ranks for {} Economics Schools for weighting "
              "according to {}".format(df.shape[0], folder.replace("_", " ")))
